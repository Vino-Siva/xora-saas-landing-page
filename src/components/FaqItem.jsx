...
const FaqItem = ({ questionItem, index }) => {
  const [activeId, setActiveId] = useState(null);
  const active = activeId === questionItem.id;
  return (
    <div className="relative z-2 mb-16">
      <div
        className="group relative flex cursor-pointer items-center justify-between gap-10 px-7"
        onClick={() => {
          setActiveId(activeId === questionItem.id ? null : questionItem.id);
        }}
      >
      ...
      <SlideDown>
        {active && (
          <div className="body-3 px-7 py-3.5">{questionItem.answer}</div>
        )}
      </SlideDown>
      <div
        className={clsx(
